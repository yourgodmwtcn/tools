function Z_PS_ini_rivers(yearrun, S, out_dir, ininame, flowFile, riverFile, riverpolyFile)
...
timeriv = dsearchn(Qr_yearday2,tt);
...
%%% now interpolate from seaward station to innermost station for each river region
 for i=1:num_rivers
   ...
   if(~strcmp('skagit_south',region)); %skip skagit south if it's there
    ...
    Temp(:,1) = squeeze(tt(:,ygrd,xgrd)); Temp(:,end) = T_riv(timeriv,i);
