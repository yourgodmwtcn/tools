function [iList, jList, sList, Constant]=...
    GRID_LinProgGetIJS_rx0(MSK, DEP, rx0matrix)
...
disp('Writing down inequalities for rx0 factor');
%
TotalNbConstant=0;
TotalNbEntry=0;
...
for iEta=1:eta_rho-1
  for iXi=1:xi_rho
    if (MSK(iEta, iXi) == 1 && MSK(iEta+1, iXi) == 1)
      ...
      %
      r=min(rx0matrix(iEta,iXi), rx0matrix(iEta+1,iXi));
      nbConst=nbConst+1;
...
for iEta=1:eta_rho
  for iXi=1:xi_rho-1
    if (MSK(iEta, iXi) == 1 && MSK(iEta, iXi+1) == 1)
      ...
      %
      r=min(rx0matrix(iEta,iXi), rx0matrix(iEta,iXi+1));
      nbConst=nbConst+1;
