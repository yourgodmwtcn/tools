function h = ZPP_smoothgrid(h,hmin,rmax)
%
...
while r>rmax
  i=i+1;
  cx = (rx>cff*rmax);
  cx = double(cx);% PM 1/25/2005 avoids error message re: numbers=>logicals
  cx=hanning(cx);
  cy=(ry>cff*rmax);
  cy = double(cy);% PM 1/25/2005 avoids error message re: numbers=>logicals
  cy=hanning(cy);
  fx=cx.*FX(h);
  fy=cy.*FY(h);
