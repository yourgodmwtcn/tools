...
diss  = -Cd*Urho.*avg1(avg1(vor,1),2)./Hrho;
...
Z_dar;
% add labels
title('DA Vorticity (s^{-1})','fontweight','bold')
...
h(2) = subplot(242)
imagescnan(G.lon_rho(2:end-1,2:end-1),G.lat_rho(2:end-1,2:end-1),slope'); shading flat
...
Z_dar;
% add labels
title('DA Slope Torque','fontweight','bold')
xlabel('Longitude (deg)')
% add coastline
Z_addcoast('detailed',Tdir.coast);
% and velocity vectors
...
h(3) = subplot(243)
imagescnan(G.lon_rho(2:end-1,2:end-1),G.lat_rho(2:end-1,2:end-1),speed'); shading flat
caxis(cvec);
hcbar = colorbar('Southoutside'); 
% fix scaling
Z_dar;
% add labels
title('DA Speed Torque','fontweight','bold')
xlabel('Longitude (deg)')
% add coastline
Z_addcoast('detailed',Tdir.coast);
set(gca,'YTickLabel',[]);
...
h(4) = subplot(244)
imagescnan(G.lon_rho(2:end-1,2:end-1),G.lat_rho(2:end-1,2:end-1),diss'); shading flat
caxis(cvec);
hcbar = colorbar('Southoutside'); 
% fix scaling
Z_dar;
% add labels
title('DA Dissipation','fontweight','bold')
xlabel('Longitude (deg)')
% add coastline
Z_addcoast('detailed',Tdir.coast);
% and velocity vectors
