...
function [mixedtp, mixedsp, mixeddp] = findmld(presx,salx,tempx,yesplot)

% i=12;
% yesplot=1;
% presxx = denan(hrp.pres(:,i));
% salxx = denan(hrp.salt(:,i));
% tempxx = denan(hrp.temp(:,i));
% 
% presx = presxx(end-100:end);
% salx = salxx(end-100:end);
% tempx = tempxx(end-100:end);
% 
presx = denan(presx);
salx = denan(salx);
tempx = denan(tempx);

minlen = min([length(presx),length(salx),length(tempx)]);

% interpolate the super high resolution hrp data onto a 5m profile
pres = (presx(1):1:presx(end))';
sal = interp1(presx(1:minlen),salx(1:minlen),pres);
temp = interp1(presx(1:minlen),tempx(1:minlen),pres);
...
mldepthptmp(mldindex) = presinterp(max(find(abs(tempthreshold)<.20001)));
...
if yesplot==1
        ...
        axis([mintemp maxtemp 0 max(presx)])
        grid on
        hold off
        ...
        set(gca, 'YDir','reverse','XAxisLocation','top')
        grid on 
        titlename = [' profile ' int2str(mldindex) ' of float '];
        title(titlename,'FontSize',18)
        xlabel('Salinity (PSU)','FontSize',14')
        axis([minsal maxsal 0 max(presx)])
        hold off
        ...
        axis([minpden maxpden 0 max(presx)])
        hold off
end
