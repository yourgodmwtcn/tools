...
west=deg180(region(1));dlon=region(1)-west;
...
if north<-80.738||south>80.738
    ...
    if (west<0)&&(east>=0)
        ...
        [mat2,lat,lon2]=read_sand1(fid,south,north,0,min(east,360-ddeg),ddeg,minlat,skip,nbytes);
        mat=[mat1 mat2];
        lon=[lon1 lon2];
    else
        [mat,lat,lon]=read_sand1(fid,south,north,west,min(east,360-ddeg),ddeg,minlat,skip,nbytes);
    end
    
     if east>360
         mat1=mat;
         lon1=lon;
         [mat2,lat2,lon2]=read_sand1(fid,south,north,360,east,ddeg,minlat,skip,nbytes);
         mat=[mat1 mat2];
         lon=[lon1 lon2];
     end
    ...
    lon=unwrap(frac(2*pi,360)*lon)*frac(360,2*pi)+dlon;
end
