...
%   (C) 2009--2012 J.M. Lilly --- type 'help jlab_license' for details
...
for i=1:length(varargin)
    if ~isreal(varargin{i})
       varargout{i}=(inf+sqrt(-1)*inf)*ones(N,M,L);
    else
       varargout{i}=inf*ones(N,M,L);  
    end
end
...
if ~isempty(ir)
    ...
    for i=1:length(varargin)
        for j=1:M
            for k=1:size(ir,2)
                 varargout{i}(ir(isfinite(ir(:,k)),k),j,k)=varargin{i}(isfinite(ir(:,k)),j,k);
            end
        end
    end
end
...
mult=vsum(isfinite(varargout{1}(:,1,:)),3);
...
reporttest('RIDGEMAP has one page per ridge, joint ridges',size(fa,3)==size(wa,3)&&size(fa,3)==length(find(~isfinite(ir))))
