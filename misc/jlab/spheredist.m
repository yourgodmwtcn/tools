...
%   circle from the point with latitude and longitude coordinates LAT1 and
%   LON1 to the point LAT2 and LON2. 
%  
%   All input arguments should be arrays of the same size, or else either
%   LAT1,LON1 or LAT2,LON2 should be scalars.
%
%   D=SPHEREDIST(LAT,LON) with two input arguments LAT and LON, where these
%   are of arrays of length N, computes the great circle distance D between
%   adjacent points.  D is then of length N-1.
%
...
%   (C) 2006--2012 J.M. Lilly --- type 'help jlab_license' for details    
...
if length(varargin)==2
    if ~isvector(varargin{1})||~isvector(varargin{2})
        error('Sorry, with 2 or 3 input arguments LAT and LON must be vectors.')
    end
